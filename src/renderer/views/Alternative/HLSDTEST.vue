<template>
  <div id="wrap">
    <div id="form">
      <div class="title">{{ localization.get("#DEBUG_HLSD") }}</div>
      <input type="file" @change="fileSelected" />
      <div class="info-table">
        <div class="cell">
          <h3 style="margin-bottom: 8px">Информация о файле</h3>
          <div class="entry">
            <div class="name">{{ localization.get("#HLSD_NAME") }}</div>
            <div class="value">{{ hlsc.name ? hlsc.name : "Значение" }}</div>
          </div>
          <div class="entry">
            <div class="name">{{ localization.get("#HLSD_DESC") }}</div>
            <div class="value">
              {{ hlsc.description ? hlsc.description : "Значение" }}
            </div>
          </div>
          <div class="entry">
            <div class="name">{{ localization.get("#HLSD_AUTHOR") }}</div>
            <div class="value">
              {{ hlsc.author ? hlsc.author : "Значение" }}
            </div>
          </div>
          <div class="entry">
            <div class="name">{{ localization.get("#HLSD_GAME") }}</div>
            <div class="value">{{ hlsc.game ? hlsc.game : "Значение" }}</div>
          </div>
          <div class="entry">
            <div class="name">{{ localization.get("#HLSD_SCRIPTLESS") }}</div>
            <div class="value">
              {{ hlsc.scriptless ? hlsc.scriptless : "Значение" }}
            </div>
          </div>
          <div class="entry">
            <div class="name">{{ localization.get("#HLSD_DATA_LEN") }}</div>
            <div class="value">
